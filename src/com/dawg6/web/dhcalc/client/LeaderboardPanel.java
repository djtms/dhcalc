...
import com.dawg6.d3api.shared.Util;
...
public class LeaderboardPanel extends ApplicationPanel {
	...
	protected void leaderboardLink() {
		...
		if ((realm != null) && (season != null) && (type != null)) {
			...
			String url = server + "/d3/rankings/"
					+ ((season < 0) ? "era" : "season") + "/"
					+ Math.abs(season) + "/" + which;
	...
	protected void getLeaderboard() {
		...
		if (type != null) {
			...
			if (season != null) {
				Service.getInstance().getLeaderboard(
						mainPanel.getSelectedRealm(), Math.abs(season),
						season < 0, which, new DefaultCallback<Leaderboard>() {
		...
		for (LeaderboardRow row : result.rows) {
			...
			if ((row != null) && (row.players != null)) {
				if (row.data != null) {
					...
					Long rift = Util.getLeaderboardDataNumber(row.data,
							"RiftLevel");
					Long time = Util.getLeaderboardDataTimestamp(row.data,
							"RiftTime");
					Long rank = Util.getLeaderboardDataNumber(row.data, "Rank");

					if ((rift != null) && (time != null) && (rank != null)) {
						for (Account a : row.players) {

							String clazz = Util.getLeaderboardDataString(
									a.data, "HeroClass");

							if ((clazz != null) && clazz.equals("demon hunter")) {

								String btag = Util.getLeaderboardDataString(
										a.data, "HeroBattleTag");
								Long heroId = Util.getLeaderboardDataNumber(
										a.data, "HeroId");

								if ((btag != null) && (heroId != null)) {
									long minutes = time / 60000L;
									long seconds = (time - (minutes * 60000)) / 1000L;
									String ts = NumberFormat.getFormat("00")
											.format(minutes)
											+ ":"
											+ NumberFormat.getFormat("00")
													.format(seconds);
									String label = rank + ") Level " + rift
											+ ": " + ts + " - " + btag;
	...
	protected void getHeroList() {
		...
		if (btag != null) {
			String[] split = btag.split("#");
			mainPanel.importProfile(split[0], split[1],
	...
	private class SeasonHolder {
		...
		public String value() {
			return (era != null) ? String.valueOf(-era) : String
					.valueOf(season);
		}
	}
}
