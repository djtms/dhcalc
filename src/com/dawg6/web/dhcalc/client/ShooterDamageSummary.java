...
public class ShooterDamageSummary extends SimpleCaptionPanel {
	...
	public ShooterDamageSummary() {
		...
		list.add(new Column<Holder>("Shooter", "dpsCol", new Comparator<Holder>(){
			...
			public int compare(Holder o1, Holder o2) {
				return o1.shooter.toString().compareToIgnoreCase(o2.shooter.toString());
			}}));
		...
		summary = new SortableTable<Holder>(new SortableTable.DefaultRenderer<Holder>(list) {
			...
			public void renderRow(SortableTable<Holder> table, int row,
				...
				table.setWidget(row, 0, new Label(item.shooter.toString(), false));
	...
	public void setData(Map<String, DamageHolder> values, double total,
		...
		for (Map.Entry<String, DamageHolder> e : values.entrySet()) {
			Holder h = new Holder();
